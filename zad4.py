...
move_camera_object = 1 # 1 - ruszanie kamera (default), 2 - ruszanie obiektem
upY = 1.0
...
def render(time):
    ...
    if move_camera_object == 1:
        ...
        if right_mouse_button_pressed:
            scale += delta_x * 0.001
            scale = max(0.2, min(scale, 4.0))  # wartosci ograniczone do przedzialu [0.2; 4.0]
...
def keyboard_key_callback(window, key, scancode, action, mods):
    global move_camera_object

    if key == GLFW_KEY_ESCAPE and action == GLFW_PRESS:
        glfwSetWindowShouldClose(window, GLFW_TRUE)
    elif key == GLFW_KEY_SPACE and action == GLFW_PRESS:
        if move_camera_object == 1:
            move_camera_object = 2
        else:
            move_camera_object = 1
